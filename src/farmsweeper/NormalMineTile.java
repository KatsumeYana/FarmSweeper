...
public class NormalMineTile extends JButton {
    int r, c;  // Row and column of the tile
    boolean isFlagged;
    boolean isRevealed;
    boolean isMine;
    private JLabel tileLabel;
    private final String theme;  // Theme of the game (Spring, Summer, Autumn)

    public NormalMineTile(int r, int c, String theme) {
        this.r = r;
        this.c = c;
    ...
    public void reveal(String theme, int adjacentMines) {
        ...
        if (isMine) {
            revealMine(theme); // Show a mine image if it's a mine
        }
    }

    // Method to reveal the mine (bomb)
    public void revealMine(String theme) {
        isRevealed = true;  
        String mineImagePath = switch (theme) {
            case "Spring" -> "resources/images/potato.png";
            case "Summer" -> "resources/images/melon.png";
            case "Autumn" -> "resources/images/pumpkin.png";
            default -> "none";
        };
        // Set the mine image
        ImageIcon mineIcon = new ImageIcon(mineImagePath);
        tileLabel.setIcon(mineIcon);
        tileLabel.setHorizontalAlignment(SwingConstants.CENTER);
        tileLabel.setVerticalAlignment(SwingConstants.CENTER);
    }
